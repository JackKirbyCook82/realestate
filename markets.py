...
class PropertyMarket(object):
    def __init__(self, max_market_movement, market_sensitivity, market_threshold, *args, households=[], housings=[], functiontype='hyperbolic_tangent', **kwargs):
        ...
        self.__marketThreshold = market_threshold
    ...
    def demand_matrix(self, utility_matrix, *args, **kwargs):
        utility_matrix = np.apply_along_axis(_zscore, 0, utility_matrix)
        utility_matrix = np.apply_along_axis(_threshold, 0, utility_matrix, self.__marketThreshold)
    ...
    def __call__(self, duration_months, *args, **kwargs): 
        ...
        for index, household in enumerate(self.__households): 
            self.__households[index] = household(duration_months, *args, **kwargs)
        for index, housing in enumerate(self.__housing):
            self.__housing[index] = housing(duration_months, *args, pricerate=priceRates[index], rentrate=rentRates[index], **kwargs)            
        return self
